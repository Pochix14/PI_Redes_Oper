...
void Socket::SSLInit() {
   this->SSLInitContext();
  SSL * ssl = SSL_new( /*SSL_CONTEXT*/ (SSL_CTX *) this->SSLContext );
  if ( NULL == ssl ) {
    perror("SSL_new");
    exit( 3 );
  }
...
void Socket::SSLInitServer( const char * certFileName, const char * keyFileName) {
  ...
  this->SSLStruct = (void *) ssl;
  this->useSSL = true;
  this->SSLLoadCertificates(/*Certificate name*/ certFileName, /*Key file name*/ keyFileName);
}

void Socket::SSLCreate( Socket * original ) {
  int st = -1;
  SSL * ssl = SSL_new( /*SSL_CONTEXT*/ (SSL_CTX *) original->SSLContext );
  if ( NULL == ssl ) {
    perror("SSL_new");
    exit( 3 );
  }
  this->SSLStruct = ssl;
  this->useSSL = true;
  if ( SSL_set_fd(/*SSL Struct*/ (SSL*) this->SSLStruct, /*Socket descriptor*/ this->idSocket) <= 0 ) {  
    st = SSL_get_error( /*SSL Struct*/ (SSL *) this->SSLStruct, /*Status*/ st );
    fprintf( stderr, "SSLCreate failed with SSL_get_error code %d\n", st );
    abort();
  }
}
...
const char * Socket::SSLGetCipher() {
  const char* name = SSL_get_cipher(/*SSL Struct*/ (SSL *) this->SSLStruct);
  return name;
}
...
void Socket::SSLAccept(){
  int st = -1;
  st = SSL_accept(/*SSL Struct*/ (SSL*) this->SSLStruct);
  if (st <= 0) {
    st = SSL_get_error( /*SSL Struct*/ (SSL *) this->SSLStruct, /*Status*/ st );
    fprintf( stderr, "SSLCreate failed with SSL_get_error code %d\n", st );
    abort();
